...
sqlTable::sqlTable(pgConn *conn, pgQueryThread *_thread, const wxString &tabName, const OID _relid, bool _hasOid, const wxString &_pkCols, char _relkind)
{
	...
	wxString tempSqlStr = wxT("SELECT n.nspname AS nspname, relname, format_type(t.oid,NULL) AS typname, format_type(t.oid, att.atttypmod) AS displaytypname, ")
		wxT("nt.nspname AS typnspname, attname, attnum, COALESCE(b.oid, t.oid) AS basetype, atthasdef, ");
	if (connection->BackendMinimumVersion(12, 0))
	{
		tempSqlStr += wxT("pg_get_expr(def.adbin, def.adrelid) AS adsrc,\n");
	}
	else {
		tempSqlStr += wxT("adsrc,\n");
	}
	tempSqlStr += wxT("       CASE WHEN t.typbasetype::oid=0 THEN att.atttypmod else t.typtypmod END AS typmod,\n")
		wxT("       CASE WHEN t.typbasetype::oid=0 THEN att.attlen else t.typlen END AS typlen\n")
		wxT("  FROM pg_attribute att\n")
		wxT("  JOIN pg_type t ON t.oid=att.atttypid\n")
		wxT("  JOIN pg_namespace nt ON nt.oid=t.typnamespace\n")
	...
	pgSet *colSet = connection->ExecuteSet(tempSqlStr);
